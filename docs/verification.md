...
All six developer-supplied captures in `../Fastline/generated/screenshots/raw/en-US` are included on the homepage. Overview appears in the hero, one-time review and results appear together in the workflow section, and Locations and Exclusions appear beside the control guidance. The current menu-bar capture appears in the background section. Captions and the FAQ identify screenshot savings as example data. The SSD-write guidance is unchanged, and the example locations do not replace the recommendation to monitor mostly stable folders.

The six source PNGs were refreshed from the supplied batch and recompressed losslessly without changing pixels or dimensions. Image inspection confirmed transparency and the original proportions in every generated WebP variant, with no upscaling. The six captures generate 26 responsive variants. Only Overview is eager and high priority. The remaining images are lazy-loaded and all have reserved dimensions and descriptive alternative text.
...
The current batch refreshes all six website assets. Overview remains 1622 by 1472 pixels, Locations and Exclusions remain 1622 by 1200 pixels, one-time review is 1280 by 1188 pixels, one-time results is 1280 by 1242 pixels, and the menu-bar capture is 720 by 1178 pixels. Each source keeps its supplied content while the optimized PNGs use lossless compression. The production build, TypeScript check, Cloudflare dry run, static checks, source-file comparisons, and local asset inspection passed. No layout, CSS-loading, or appearance-control behavior changed.
